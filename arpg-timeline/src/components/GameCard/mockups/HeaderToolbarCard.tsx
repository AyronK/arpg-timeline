...
/**
 * MOCKUP ONLY — a header-positioned variant of the real GameCard's action toolbar (Builds,
 * Guides, Overview, More). Deliberate visual fork of GameCard.tsx (same real GameMenu,
 * CommunityLabel, SteamPlayersChip, UTM/analytics wiring — this is a layout comparison, not a new
 * concept), not a shared component: the toolbar sits above the title/badges instead of pinned to
 * the bottom, with the divider on the bottom edge and rounded top corners instead of bottom.
 */
...
export const HeaderToolbarCard = ({
    name,
    gameLogo,
    url,
    buildsUrl,
    ...
    const hasHeaderActions = Boolean(buildsUrl) || Boolean(guidesUrl) || showOverviewAndMenu;
    const logoContent = (
    ...
    return (
        <section className="bg-card text-card-foreground relative flex flex-1 flex-col gap-1 rounded-md border p-4">
            {hasHeaderActions && (
                <div className="border-foreground/10 divide-foreground/10 -mx-4 -mt-4 flex h-8 flex-row divide-x overflow-hidden rounded-t-md border-b">
